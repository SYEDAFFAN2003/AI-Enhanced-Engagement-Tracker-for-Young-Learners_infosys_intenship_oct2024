...
p = "C:/Users/syeda/OneDrive/Documents/Custom Office Templates/face_recognition/facial-landmarks-recognition/shape_predictor_68_face_landmarks.dat"
detector = dlib.get_frontal_face_detector()
landmark_predictor = dlib.shape_predictor(p)

# Create a directory to save screenshots
if not os.path.exists("AFFAN_screenshots(8)"):
    os.makedirs("AFFAN_screenshots(8)")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"AFFAN_screenshots(8)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive == 'Yes':
        ...
        if time.time() - start_time >= 30:
            ...
            if not df.empty:
                df.to_excel('AFFAN_attendance_with_attention_and_average(8).xlsx', index=False)
                print("Attendance saved to 'AFFAN_attendance_with_attention_and_average(8).xlsx'.")
    ...
    if not df.empty:
        ...
        df.to_excel('AFFAN_attendance_with_attention_and_average(8).xlsx', index=False)
        print("Attendance with attentiveness and average score saved to 'teja_attendance_with_attention_and_average(8).xlsx'.")
